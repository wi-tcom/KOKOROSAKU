# SAKU Trainer vNext — Usage & Concept

> ツール: `tools/saku-trainer-vnext.html`。評価エンジンは `tools/vnext/derived-profile-engine.mjs`（Builder と共有）。
...
内部検証: `node scripts/verify_vnext.mjs`（LOOP-01..06）でこの一周が成立することを担保。
