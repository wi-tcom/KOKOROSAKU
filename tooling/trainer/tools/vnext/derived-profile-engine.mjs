// SAKU vNext — Derived Character Profile Engine
// -------------------------------------------------------------------------
// Character 設計値（vNext Canonical）から「予測される Character 傾向」を導出する。
...
//
// 重要な設計不変条件（verify_vnext.mjs が反証テストする）:
...
export function generatePrompt(c, platform = "generic") {
  ...
  const body = [
    ...
    `表現: ${ax.e_vocabulary_tone} / ${ax.c_intelligence_vector} / 応答リズム ${ax.g_pulse}。`,
