...
import {
  ...
  listExternalReviews,
  storeExternalReview,
} from "./external-review-intake.mjs";
import { TUNING_ITEMS } from "./tuning-projection.mjs";
...
function header() {
  return `<header><div class="brand">SAKU <strong>EXTERNAL REVIEW</strong></div><div class="header-actions"><a id="to-home" class="button-link" href="../index.html?stay=1">${t("ホーム", "Home")}</a><a id="open-existing-trainer" class="button-link" href="./trainer.html">${t("既存の Trainer を開く（別操作）", "Open the existing Trainer (separate action)")}</a><label>${t("表示言語", "Interface language")}<select id="locale"><option value="ja" ${language === "ja" ? "selected" : ""}>日本語</option><option value="en" ${language === "en" ? "selected" : ""}>English</option></select></label></div></header>`;
}
