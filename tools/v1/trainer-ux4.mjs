...
import { createSession as frozenSession, stableStringify } from './trainer-frozen-ia.mjs';
import { TUNING_KNOWLEDGE } from '../unified-v1/tuning/tuning-projection.mjs';
import { HANDOFF_FORMAT, PROMPT_CONSTRUCTION, TRAINING_MENU_SEPARATOR, platformLaunchText, trainerHandoffText } from '../unified-v1/platform-prompt.mjs';
...
async function deterministicRecommendation(s, result) {
  ...
  return {
    ...
    translation_contract: 'tools/unified-v1/tuning/tuning-projection.mjs#translate',
    tuning_measurements: [],
...
async function customOperation(input, op) {
  ...
  if(op.type==='menu-generate'){
    ...
    const candidate={id,version:AI_CANDIDATE_VERSION,source_response_id:sourceId,result_id:result.id,evaluation_id:evaluation.id,question_attempt_id:result.question_attempt_id,
      ...
      translation_rule_version:'tools/unified-v1/tuning/tuning-projection.mjs#translate',knowledge_version:TUNING_KNOWLEDGE.version,knowledge_digest:await digest(TUNING_KNOWLEDGE),
